...
import com.example.luteforandroidv2.network.KokoroTTSClient
...
class TTSManager(private val context: Context) {
    ...
    // Kokoro TTS client
    private var kokoroClient: KokoroTTSClient? = null
    ...
    private var selectedTTSEngine: String = "Android TTS" // Default to Android TTS
    private var kokoroServerUrl: String = "http://192.168.1.100:8880" // Default URL
    ...
    fun initializeTTS(initCallback: (Boolean) -> Unit) {
        ...
        if (selectedTTSEngine == "Kokoro TTS") {
            ...
            kokoroClient = KokoroTTSClient(context).apply {
                setTTSClientCallback(object : KokoroTTSClient.TTSClientCallback {
                    override fun onTTSStateChanged(isPlaying: Boolean) {
                        ttsListener?.onTTSStateChanged(isPlaying)
                    }

                    override fun onError(error: String) {
                        ttsListener?.onError(error)
                    }
                })
            }
            isInitialized = true
    ...
    fun pause() {
        ...
        if (selectedTTSEngine == "Kokoro TTS") {
            ...
            // Pause Android TTS - note: TextToSpeech doesn't have true pause functionality
    ...
    fun isSpeaking(): Boolean {
        return if (selectedTTSEngine == "Kokoro TTS") {
            kokoroClient?.isSpeaking() == true
        } else {
            isInitialized && tts != null && tts?.isSpeaking == true
        }
    }
    ...
    fun setLanguage(languageCode: String): Boolean {
        ...
        if (selectedTTSEngine == "Kokoro TTS") {
            // Kokoro doesn't support language switching in the same way as Android TTS
            // The language is usually determined by the text content
            return true
        } else {
    ...
    fun getAvailableVoices(): List<String> {
        ...
        if (selectedTTSEngine == "Kokoro TTS") {
            // For Kokoro, return a list of common voice names
            return listOf("af_bella", "af_heart", "af_sky", "af_iris", "af_echo")
        } else {
