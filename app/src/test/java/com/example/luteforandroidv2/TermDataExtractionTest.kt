...
Gg</textarea></div>
  ...
  <script>
    ...
    fun testExtractTermText() {
        ...
        assertEquals("click", matcher.group(1))
    }

    @Test
    fun testExtractTranslation() {
        val pattern =
                Pattern.compile(
        ...
        val matcher = pattern.matcher(htmlContent)
        assertTrue(matcher.find())
        assertEquals("Gg", matcher.group(1))
    }

    @Test
    fun testExtractParent() {
        val pattern =
                Pattern.compile(
        ...
        val matcher = pattern.matcher(htmlContent)
        assertTrue(matcher.find())
        assertEquals("mm", matcher.group(1))
    }
    ...
    fun testExtractLanguageId() {
        val pattern = Pattern.compile("""<option selected value="(\d+)">.*?</option>""")
        val matcher = pattern.matcher(htmlContent)
        assertTrue(matcher.find())
    ...
    fun testExtractSyncStatus() {
        ...
        val matcher = pattern.matcher(htmlContent)
        assertTrue(matcher.find())
        assertEquals("y", matcher.group(1))
    }

    @Test
    fun testExtractImage() {
        val pattern =
                Pattern.compile(
                        """<input id="current_image" name="current_image" type="hidden" value="(.*?)">"""
                )
